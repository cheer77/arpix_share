...
Щоб оновити сайт на GitHub Pages (запушити зміни), використовуйте тільки одну команду:

```bash
npm run deploy
```

Ця команда сама збере проект (`build`) і відправить його у правильну гілку `gh-pages` для оновлення сайту в інтернеті. Після запуску зачекайте кілька хвилин, поки GitHub оновить сторінку.
